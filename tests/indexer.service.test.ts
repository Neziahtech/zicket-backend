...
describe('IndexerService', () => {
  beforeEach(() => {
    ...
    (IndexerService as any).contractAddress = process.env.INDEXER_CONTRACT_ADDRESS?.toLowerCase();
  ...
  it('should sync events from Soroban RPC and insert into MongoDB', async () => {
    ...
    expect(IndexerState.findOne).toHaveBeenCalledWith({ contractAddress: process.env.INDEXER_CONTRACT_ADDRESS?.toLowerCase() });
    
    expect(ContractEvent.updateOne).toHaveBeenCalledWith(
      { transactionHash: 'tx123', ledgerSequence: 950, eventIndex: 1 },
      expect.objectContaining({ $setOnInsert: expect.objectContaining({ eventName: 'TicketMinted', args: { ticketId: 1, owner: 'GBABC' } }) }),
      { upsert: true }
    );
  });
  ...
  it('should resume from lastIndexedLedger after simulated restart', async () => {
    ...
    const contractAddress = process.env.INDEXER_CONTRACT_ADDRESS?.toLowerCase() || '';
    ...
    expect(mockServer.getEvents).toHaveBeenCalledWith(expect.objectContaining({
      startLedger: 1001,
    }));

    expect(ContractEvent.updateOne).toHaveBeenCalledWith(
      { transactionHash: 'tx456', ledgerSequence: 1010, eventIndex: 1 },
      expect.objectContaining({ $setOnInsert: expect.objectContaining({ eventName: 'PaymentProcessed', args: { amount: 100 } }) }),
      { upsert: true }
    );
  });
});
